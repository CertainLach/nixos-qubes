...
in
{
  ...
  config = mkIf cfg.dom0.enable {
    ...
    # FIXME(upstream): qubes gid should be added
    users.groups.qubes.gid = config.ids.gids.qubes or 999;
    ...
    services.usbguard = mkIf (isDom0 && recommended "dom0-restricted-usb") {
      ...
      # FIXME(upstream): AuditBackend is not configurable by NixOS module for now
      ...
      # FIXME(upstream): usbguard module doesn't support rules.d, as symlinkJoin doesn't support
      # symlinkJoin...
      # Needs https://github.com/NixOS/nixpkgs/pull/345127
      # packages = [
