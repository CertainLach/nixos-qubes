{lib, config, pkgs, ...}:
let
	inherit (lib) mkEnableOption mkOption mkIf nameValuePair mapAttrs';
	inherit (lib.types) attrsOf path submodule str;
...
in
{
	...
	config = mkIf cfg.enable {
		...
		systemd.services = mapAttrs' cfg.templates (name: template: nameValuePair "qubes-oobe-template-${name}" {
		...
		}) // mapAttrs' cfg.kernels (name: kernel: nameValuePair "qubes-oobe-kernel-${name}" {
