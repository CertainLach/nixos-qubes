...
let
  ...
  mkVgaBios =
    ...
    mkBios {
      ...
      outBin = "vgabios.bin";
      name = "vgabios-${name}.bin";
      target = "out/vgabios.bin";
    };
  ...
  seabioses = concatStringsSep "," [
    (mkBios {
      config = "config.seabios-128k";
      outBin = "bios.bin";
      name = "bios.bin";
    })
    (mkBios {
      config = "config.seabios-256k";
      outBin = "bios.bin";
      name = "bios-256k.bin";
    })
    (mkBios {
    ...
    (mkBios {
      config = "config.coreboot";
      outBin = "bios.bin.elf";
      name = "bios-coreboot.bin";
    })
    (mkBios {
      config = "config.seabios-microvm";
      outBin = "bios.bin";
      name = "bios-microvm.bin";
    })
  ];
  # Probably we don't need all of those... Building for completeness.
  seavgabioses = concatStringsSep "," (
    map mkVgaBios [
      "bochs-display"
      "cirrus"
      "isavga"
